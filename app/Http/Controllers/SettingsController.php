...
class SettingsController extends Controller
{
    ...
    public function saveBusinessSettings(Request $request)
    {
...

        {
            ...
            if (!empty($request->title_text) || !empty($request->metakeyword) || !empty($request->metadesc) || !empty($request->theme_color) ||!empty($request->cust_theme_bg) ||!empty($request->cust_darklayout) || !empty($request->SITE_RTL)) {
                ...
                // $post['theme_color'] = $request->has('theme_color') ? $request-> theme_color : 'theme-3';
                // $post['is_sidebar_transperent'] = (!empty($request->is_sidebar_transperent)) ? 'on' : 'off';
                // $post['dark_mode'] = (!empty($request->dark_mode)) ? 'on' : 'off';
                if(!isset($request->cust_darklayout))
                {
                        $post['cust_darklayout']='off';
                }
    ...
    //             $company_small_logo = !empty($request->company_logo_light) ? $smallLogoName : 'small_logo.png';
